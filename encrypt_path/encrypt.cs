/**
 * Notes:
 * This was largely taken from https://social.technet.microsoft.com/Forums/office/en-US/2e0aa2b5-6256-4b05-9ef7-3d3904438fbb/where-are-photo-screensaver-settings-saved?referrer=http://social.technet.microsoft.com/Forums/office/en-US/2e0aa2b5-6256-4b05-9ef7-3d3904438fbb/where-are-photo-screensaver-settings-saved?referrer=http://social.technet.microsoft.com/Forums/office/en-US/2e0aa2b5-6256-4b05-9ef7-3d3904438fbb/where-are-photo-screensaver-settings-saved#71834aa1-45a1-4833-8410-d1198b47ee8a
 * Thank you SalvaG!
...
public static class PIDLUtils
{
        ...
        public static string Encode(string myClearPath)
        {
            IShellFolder folder;
            IntPtr pidl;
            string myEncryptedPath;
            myEncryptedPath = "ERROR";

            // Get the Desktop Folder to parse details
            if (SHGetDesktopFolder(out folder) == 0)
            {
                folder.ParseDisplayName(IntPtr.Zero, IntPtr.Zero, myClearPath, 0, out pidl, 0);
                ...
                {
                    myEncryptedPath = sb.ToString();
                }
                Marshal.FreeCoTaskMem(pidl);
            }
        
            return myEncryptedPath;
        }
        ...
        public static string Decode(string myEncryptedPath)
        {
            ...
            if (SHGetDesktopFolder(out folder) == 0)
            {
                ...
                CryptStringToBinary(myEncryptedPath, myEncryptedPath.Length, CRYPT_STRING_BASE64, IntPtr.Zero, ref largo, ref a, ref b);
                IntPtr pidl2 = Marshal.AllocCoTaskMem(largo);
                StringBuilder sb = new StringBuilder();
                if (CryptStringToBinary(myEncryptedPath, myEncryptedPath.Length, CRYPT_STRING_BASE64, pidl2, ref largo, ref a, ref b))
                {
                    //sb.Clear(); Only works with .NET 4
					sb.Length = 0;
                    sb.Capacity = 261;
                    SHGetPathFromIDList(pidl2, sb);
                    DecryptedPath = sb.ToString();
                }
